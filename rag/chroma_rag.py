...
class ChromaRAG:
    ...
    def retrieve(
        ...
        try:
            ...
            results = self.collection.query(
                query_texts=[query], n_results=k, where=filters
            )
            elapsed_seconds = time.perf_counter() - start_time
            logger.info(
                f"⏱️  retrieve(): {elapsed_seconds:.2f}s (vector search for top-{k})"
            )
    ...
    def retrieve_with_scores(self, query: str, k: int = 5) -> List[Dict[str, Any]]:
        ...
        elapsed_seconds = time.perf_counter() - start_time
        logger.info(
            f"⏱️  retrieve_with_scores(): {elapsed_seconds:.2f}s (including formatting)"
        )
