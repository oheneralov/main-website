...
class RAGPipeline:
    ...
    def __init__(self, rag_system: ChromaRAG, config: Optional[RAGConfig] = None):
    ...
    def _init_llm(self) -> Optional[Any]:
        ...
        try:
            if self.config.llm_provider == LLMProvider.LOCAL_HUGGINGFACE:
                ...
                logger.info(
                    f"🤖 LLM initialized: Local model (model: {self.config.llm_model.value}, device: {device})"
                )
                ...
                logger.info(
                    f"🤖 LLM initialized: Ollama (model: {self.config.llm_model.value}, base_url: {self.config.ollama_base_url})"
                )
                return llm
                ...
                logger.info(
                    f"🤖 LLM initialized: OpenAI (model: {self.config.llm_model.value})"
                )
                return llm
                ...
                logger.info(
                    f"🤖 LLM initialized: Anthropic Claude (model: {self.config.llm_model.value})"
                )
                return llm
            else:
                raise ValueError(
            ...
            logger.warning(
                "Install required packages: pip install langchain-openai langchain-anthropic"
            )
            return None
    ...
    def augment_query(self, query: str, k: int = 5) -> Dict[str, Any]:
        ...
        logger.info(
            f"⏱️  augment_query(): {elapsed_seconds:.2f}s (retrieval + formatting)"
        )
    ...
    def generate_response(self, query: str, k: int = 5) -> str:
        ...
        if not self.llm:
            raise RuntimeError(
                "LLM not initialized. Please install required LLM packages."
            )
...
Answer:"""
        ...
        logger.info(
            f"Invoking LLM ({self.config.llm_provider.value}) with model: {self.config.llm_model.value}"
        )
        ...
        logger.info(
            f"⏱️  generate_response(): total {total_seconds:.2f}s (augment: {augment_seconds:.2f}s, llm: {llm_seconds:.2f}s)"
        )
